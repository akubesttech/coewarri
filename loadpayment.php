...
if(empty($matx)){$pn = $apno1; $ft = "8";}else{$pn = $matx; $ft = "1";} 
 ...
 $duep1 = getDueamt($ft,$sprog,$entrymodel,$scan,0,$admitsec);
...
$que_paid = "select SUM(paid_amount) as samount,ft_cat from payment_tb where session ='".safee($condb,$admitsec)."' and pay_status='1' and  level = '".safee($condb,$entrymodel)."' and prog = '".safee($condb,$sprog)."' ";
 ...
 $sumpay = getpayamt($pn,$fc,$sprog,$entrymodel,$admitsec); //$warning_data['samount']; 
  $duep = getDueamt($fc,$sprog,$entrymodel,$scan,0,$admitsec);
...
$dpay = getDueamt($ft_cat,$program,$level,$Cat_fee,$feetype,$admitsec);
...
$queryin = mysqli_query($condb,"insert into feecomp_tb(regno,feetype,prog,level,f_amount,fcat,penalty,session,Batchno,bcode,pstatus)values('".safee($condb,$apno1)."','".safee($condb,$feetype)."','".safee($condb,$program)."','".safee($condb,$level)."','".safee($condb,$finalpay)."','".$ft_cat."','".$pen1."','".safee($condb,$admitsec)."','".safee($condb,$batchno)."','".safee($condb,$bcode)."','0')")or die(mysqli_error($condb));
}else{ $resultapp = mysqli_query($condb,"UPDATE feecomp_tb SET f_amount ='".safee($condb,$finalpay)."',session = '".safee($condb,$admitsec)."',Batchno = '".safee($condb,$batchno)."'  where feetype ='".safee($condb,$feetype)."' and prog = '".safee($condb,$program)."' and fcat = '".safee($condb,$ft_cat)."' and penalty = '".$pen1."' and level ='".safee($condb,$level)."' and regno = '".safee($condb,$apno1)."' and bcode = '".safee($condb,$bcode)."' and pstatus = '0'")or die(mysqli_error($condb));
}}else{
$queryin = mysqli_query(Database::$conn,"insert into feecomp_tb(regno,feetype,prog,level,f_amount,fcat,penalty,session,Batchno,bcode,pstatus)values('".safee($condb,$apno1)."','".safee($condb,$feetype)."','".safee($condb,$program)."','".safee($condb,$level)."','".safee($condb,$finalpay)."','".$ft_cat."','".$pen1."','".safee($condb,$admitsec)."','".safee($condb,$batchno)."','".safee($condb,$bcode)."','0')")or die(mysqli_error($condb));
}
...
//-----------------------------------------------load fees
 ...
 $fee_query = "select * from fee_db where level = '".safee($condb,$entrymodel)."' and session = '".safee($condb,$admitsec)."' and program = '".safee($condb,$sprog)."' ";
 ...
 $queryp = "select * from fee_db where level= '".safee($condb,$entrymodel)."' and session = '".safee($condb,$admitsec)."' and program='".safee($condb,$sprog)."' ";
...
<?php } ?>
Please Choose Payment plan and Click <strong> Add Payment </strong> Button to continue.
...
You'ar About to Pay acceptance Fee to Validated your Admission, Click <strong> Add Payment </strong> Button to continue.<?php } ?>
 <br> Note that Once Acceptance Fee is Successful you can pay other FEES</p>
	...
	<h4 class="panel-title"><!--Listed Below are Your Due Payment(s) --!>Please Choose payment plan </h4>
       ...
       <button name="full" class="btn btn-default" id="fbtn" data-placement="right" type="button" onclick='fullpay()' title="Click to make full payment " >Full Payment</button>	
			 			...
			 			<div class="panel-body" id="paymain">
...
$dpay = getDueamt($Fee_cat,$proga,$entrymodel,$cat,$Fee_type1,0,$admitsec);
...
<td><?php echo $number; ?> </td>
   ...
   <?php $number++;  $sumcredit += $t_namount; } ?>
   
   <tr>
<td width="30" style="text-align:centre;"  >
<input id="optionsCheckbox" class="uniform_on13" name="selectorc[]" type="checkbox" payamt="<?php echo $t_namount; ?>"   value="<?php echo $row_s['fee_id']; ?>" disabled >  
</td> 
<td><?php echo "1"; ?></td>
    <td><?php if($statuspay > 0 AND $spay >= $dpay){ echo "<div style='color:#00ff00;'>".getfeecat($Fee_cat)." ".$noteo." </div>   ";
	}else{echo "<div style='color:#ff0000;'>".getfeecat($Fee_cat)." ".$noteo."</div> ";} ?>  </td>
    <td style="text-align:center;"><strong> <span id="tot2"><?php if($sumcredit > 0){ echo number_format($sumcredit,2);}else{echo "0.00";} ?></span></strong></td>
    <td><?php echo $status; ?></td>
    </tr>
    
   
  <?php  }else{ $sumcredit = 0; ?>
  ...
  <!-- Please Note that the Minimum Payment installment is <b> --!><?php //echo "&#8358; ".number_format($inst2,2)."</b> for $enableinst installments";
   ...
   <?php } } ?>
</table> <div id="btns"><?php  if(empty($matx)){ ?>	
