...
$replacedept    = str_replace(" ","_",  $feename);
    header("Content-Type: application/xls"); 
	header("Content-Disposition: attachment; filename='$session'_'".$replacedept."'_Payment_Summary.xls");  
...
if(empty($plevel)){   $ntitle = $session." Academic Session  ";}else{ $ntitle =  getlevel($plevel,$class_ID)." ".$session."  Academic Session ";}
...
$output .="<center>
<table border='1'>";
$output .= "<tr ><td colspan='10' style='background-color: white;text-align: center;color:black;'><div style='font-size: 25px;'>".$schoolName."</div><div style='font-size: 20px;color:green;'>General Payment Summary</div>
<div style='font-size: 20px;'>".$feename."<div><div style='font-size: 20px;'>".$ntitle."<div></td></tr>";
$output .= "<thead><tr><th>S/N</th>";
...
$output .= " <th>Session</th><th>No of Installments</th> <th>Schedule Fee</th> <th>Amount Paid</th><th>Bal</th></tr></thead><tbody>";
...
$vquery .= " ORDER by fdate_paid DESC";  
}else{
$vquery = "select SUM(paid_amount) as pay_amt,count(stud_reg) as inscout,stud_reg,department,session,level,pay_status,ft_cat from payment_tb WHERE ft_cat = '".safee($condb,$fcat)."' AND prog = '".safee($condb,$class_ID)."' AND pay_status = '1' ";
...
$vquery .= " GROUP BY stud_reg,department order by stud_reg ASC,department ASC";}
...
$is_active = $fetch['pay_status'];
 $dept23 = $fetch['department']; 
$student_reg = $fetch['stud_reg'];
...
$fetch2 = mysqli_fetch_array($forderquery);
$stud_cat = $fetch2['stud_cat']; $feetype = $fetch2['fee_type']; $appno = $fetch2['app_no'];
if(substr($feetype,0,1) == "B"){ $feet = getfeecat($fetch2['ft_cat']);}else{ $feet = getftype($fetch2['fee_type']);}
if(empty($student_reg)){$matn = $appno;}else{ $matn = $fetch['stud_reg'];} 
    if(empty($student_reg)){$fullname = getappname($appno);}else{ $fullname = getname($student_reg);} 
 if($is_active == "1"){$amtn = $fetch['pay_amt']; }else{ $amtn = "0.00"; }
//get Sccheduled Fee
$namount = getDueamt($fcat,$class_ID,$fetch['level'],$stud_cat,0,$fetch['session']);
...
$output .= "<tr><td>".$noi++."</td>	
                            ...
                            <td>".getlevel($fetch['level'],$class_ID)."</td><td>".$fetch['session']."</td><td>".$fetch['inscout']."</td><td>".number_format($namount,2)."</td>
...
$output .= "<tr><td>".$noi++."</td>
                            ...
                            <td>".$phone."</td><td>".getftype($fetch['feen'])."</td><td>".$fetch['session']."</td><td> - </td><td>".number_format($namount,2)."</td>
                            <td>".number_format($amtn,2)."</td><td>".$nbal."</td></tr>"; $total += $amtn; $tbal += $bal; $tdue += $namount;}   
                                 }
          ...
          $output .= "<tr>  <td colspan='7'><strong>Total :</strong></td> <td><strong>" .number_format($tdue,2) ."<strong></td><td><strong>" .$amt ."<strong></td><td><strong>" .$fbal ."<strong></td> </tr>";
     	$output .= "";
    	if($countrow < 1){
    $output .= "<tr><td colspan='10'>No Payment Record(s) Found!</td></tr>"; }
