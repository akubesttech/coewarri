...
$duepm = getDueamt($ft,$student_prog,$student_level,$scan,0,$default_session);
if($laspdate2 < 1 ){ $catp = "1";}else{ if($sumpaym >= $duepm){ $catp = "1"; }else{  $catp = "8"; } }
$qcomp = "select * from fee_db where  level= '".safee($condb,$student_level)."' and session = '".safee($condb,$default_session)."' and program='".safee($condb,$student_prog)."' and cat_fee ='".safee($condb,$scan)."' and status = '1'";
...
$apay = getpayamt($stud_row['RegNo'],$ft_cat,$program,$nlev,$nsec,$feetype);
$dpay = getDueamt($ft_cat,$program,$nlev,$scan,$feetype);
if($apay < $dpay){ $finalpay = $dpay - $apay; }else{  $finalpay = $penaltysum;   }
$qfcomp=mysqli_query($condb,"select * from feecomp_tb where feetype ='".safee($condb,$feetype)."' and session = '".safee($condb,$nsec)."' and prog = '".safee($condb,$program)."' and fcat = '".safee($condb,$ft_cat)."' and level ='".safee($condb,$nlev)."' and regno = '".safee($condb,$stud_row['RegNo'])."'") or die(mysqli_error($condb));
